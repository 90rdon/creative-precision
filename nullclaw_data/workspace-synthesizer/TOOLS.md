...
- **Read/Write** the Expert's `IDENTITY.md` at `/root/.nullclaw/workspace-expert/IDENTITY.md` for autonomous updates
- **Read only** the Expert's `SOUL.md` — never write to it without Admin approval

### Script Execution
- Run `npx tsx src/api/nullclaw/gtm_synthesis_generator.ts` for automated report aggregation
