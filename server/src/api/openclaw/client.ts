/**
 * OpenClaw Expert Client
 *
 * Architecture Note (Updated 2026-03-02):
 * This client now uses the native OpenClaw Gateway HTTP API (v1/responses).
 * Agent logic (system.md, heartbeats, tools) lives in the gateway agents.
 * The proxy server only facilitates the communication between the UI and OpenClaw.
 * 
 * Session Management: 
 * We use OpenClaw's native session handling by providing a session_id.
 * OpenClaw persists the history internally per session.
 */
...
export class OpenClawClient {
    private static get baseUrl() {
        ...
        const url = process.env.OPENCLAW_API_ENDPOINT || 'http://openclaw-kube:18790';
        return url.replace(/\/api\/v1\/?$/, '');
    }

    private static get authToken() {
        // Auth token from openclaw.json gateway.auth.token
        return process.env.OPENCLAW_TOKEN || '09b9ddbc0845b3525a9ea2dffe4a0a87b1c94676ab791b83';
    }
     ...
     * With OpenClaw's Responses API, we don't strictly need a "create" call,
    ...
    static async createThread(): Promise<string> {
        ...
        console.log(`[OpenClawClient] Initialized session ID: ${threadId}`);
        return threadId;
    }

    /**
     * Stub for API compatibility — SessionManager handles message storage if needed,
     * but OpenClaw also keeps history internally when session_id is provided.
     */
    static async addMessage(threadId: string, content: string): Promise<void> {
        ...
        // but we'll log it for debugging and pass it to OpenClaw in the next call.
        console.log(`[OpenClawClient] Queueing message for session ${threadId}`);
    }

    /**
     * Start a streaming run via OpenClaw Gateway.
    ...
    /**
     * Stream response from the Expert agent via OpenClaw Gateway.
    ...
    static async streamResponse(sessionId: string, content: string): Promise<ReadableStream<Uint8Array> | null> {
        ...
        console.log(`[OpenClawClient] Sending request to ${this.baseUrl}/v1/responses for session ${sessionId}`);
        ...
        if (!response.ok) {
            ...
            throw new Error(`[OpenClawClient] Gateway call failed (${response.status}): ${error.slice(0, 200)}`);
        }
