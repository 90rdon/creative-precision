 ...
 * @desc    Initialize a new OpenClaw expert thread for a browser session
 */
router.post('/init', async (req, res) => {
    try {
        ...
        if (!threadId) {
            // Create new thread in OpenClaw
            threadId = await OpenClawClient.createThread();
 ...
 * @desc    Forward user message to OpenClaw and stream response
 */
router.post('/message', async (req, res) => {
    try {
        ...
        // Stream response from Expert via OpenClaw Gateway
        const stream = await OpenClawClient.streamResponse(threadId, content);
