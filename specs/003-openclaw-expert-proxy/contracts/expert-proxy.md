# Interface Contract: OpenClaw Expert Assessment Proxy
...
Called exactly once when the web page loads (or when `localStorage` has no session) to start a new OpenClaw expert thread.
...
Called every time the user sends a message. The proxy expects the `sessionId` so it knows which OpenClaw thread to forward it to.
...
The proxy will pipe the response directly from the OpenClaw API back as a raw readable stream (Server-Sent Events or Fetch Stream), allowing the frontend to animate the token generation.

## 2. Proxy to OpenClaw Interface

The Node.js proxy forwards requests securely to the OpenClaw local or remote instance. Let's assume a standard REST integration.

### 2.1 Start Thread
- **Proxy does**: `POST /api/threads` (creating an OpenClaw conversation container).
...
- **Proxy does**: `POST /api/threads/:thread_id/runs` asking OpenClaw to stream a response.
