# Quickstart: Testing the OpenClaw Expert Assessment Proxy

This feature bridges the React frontend to the OpenClaw agent backend using a Node.js proxy server.

### Prerequisites (Local Env)
- Node.js running (`npm run dev`) for both frontend and proxy.
- OpenClaw running locally (accessible via its HTTP API endpoint).
...
1. **Start the local cluster**:
   ...
   Ensure OpenClaw is running and reachable.
...
5. **Test Seamless Streaming**:
   ...
   Refresh the page. Wait for the state to reload. It should fetch your `sessionId` from `localStorage` and reconstruct the chat history from the same OpenClaw thread.
