...
async function logAgentChange(tier: 1 | 2, title: string, reason: string, diff: string) {
    ...
    if (tier === 1) {
        ...
        // The exact telegram call might use the nullclaw messaging features, 
        // but we can just ask the NullClaw agent to print a message to the channel or call its chat interface
