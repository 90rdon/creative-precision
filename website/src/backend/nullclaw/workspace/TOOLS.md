...
- **Log Executive Insight**: The function `logExecutiveInsight` in `src/api/nullclaw/telemetry.ts`. Use this to log insights gathered after a chat string via the Guarded Proxy.
- **Store Market Signals**: The function `storeAutomatedMarketResearch` in `src/api/nullclaw/gtm_synthesis_generator.ts`. Call this when you perform periodic background web searches to record sentiment data into Supabase (`market_signals`).
