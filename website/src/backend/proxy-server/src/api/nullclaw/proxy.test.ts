...
describe('POST /api/assessment/init', () => {
    ...
    it('should create a new session and return sessionId when none exists', async () => {
        ...
        vi.mocked(NullClawClient.createThread).mockResolvedValue('thread_new123');
        ...
        expect(NullClawClient.createThread).toHaveBeenCalledOnce();
    ...
    it('should fire a Telegram alert only for NEW sessions', async () => {
        vi.mocked(SessionManager.getThreadId).mockResolvedValue(undefined);
        vi.mocked(NullClawClient.createThread).mockResolvedValue('thread_xyz');
    ...
    it('should return 500 when NullClaw createThread fails', async () => {
        vi.mocked(SessionManager.getThreadId).mockResolvedValue(undefined);
        vi.mocked(NullClawClient.createThread).mockRejectedValue(new Error('Connection refused'));
